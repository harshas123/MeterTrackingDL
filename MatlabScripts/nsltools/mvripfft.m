...
% modulation
S = [S; 0];
S(max(1, fdx-Ri)) = S(max(1, fdx-Ri)) + r.*exp(1i*(th-ph));	% lower side
S(min(T1*SF/2+1, fdx+Ri)) = S(min(T1*SF/2+1, fdx+Ri)) + r.*exp(1i*(th+ph));	% upper side
S = S * Am/2;
S(1) = 0; S = S(1:T1*SF/2);

% original stationary spectrum
S(fdx) = S(fdx) + r.*exp(1i*th);  % moved here to save computation
