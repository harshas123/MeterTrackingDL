function [v5,fbank_resp] = wav2aud(x, paras, filt, VERB, op_stage)
...
if (fac ~= -2)
    y2 = filter(1, [1 -beta], y2);
end
y2_h = y2;
y3_h = 0;

t0 = clock;
fbank_resp = zeros(length(y2),M);
if op_stage>1
    fbank_resp(:,M) = y2;
else
    fbank_resp(:,M) = y1;
...
for ch = (M-1):-1:1
    ...
    elseif op_stage==4
        fbank_resp(:,ch) = y4;
    elseif op_stage==5
        fbank_resp(1:N,ch) = v5(:, ch);
    end

%     fbank_resp(:,ch) = y4;
    
	if VERB && strcmp(filt, 'p') 
        if rem(ch, 24) == 0
            if exist('octband')
                octband = octband + 1; 
            else
